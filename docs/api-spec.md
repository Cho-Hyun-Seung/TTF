...
- 대상: React 프론트엔드와 별도 구현되는 TTF 백엔드
- Base path: `/api/v1`
...
- `room_id`, `participant_id`, `round_id`, `statement_id`는 외부에서 추측하기 어려운 불투명 문자열이다. UUIDv4/UUIDv7 사용을 권장하지만 클라이언트는 UUID 형식에 의존하지 않는다.
...
- 서버 스냅샷의 `version`은 방마다 단조 증가하는 정수다. 게임 상태에 영향을 주는 쓰기가 커밋될 때 증가한다.
...
```http
Set-Cookie: ttf_host_session=<opaque>; Path=/api/v1/rooms/<room_id>; HttpOnly; Secure; SameSite=Lax
Set-Cookie: ttf_participant_session=<opaque>; Path=/api/v1/rooms/<room_id>; HttpOnly; Secure; SameSite=Lax
```
...
| Method | Path | 권한 | 설명 |
|---|---|---|---|
| `POST` | `/rooms` | 공개 | 방 생성 및 진행자 세션 발급 |
| `GET` | `/rooms/by-code/{code}` | 공개 | 입장 전 방 정보 조회 |
| `POST` | `/rooms/{room_id}/participants` | 공개 | 닉네임으로 참가 및 세션 발급 |
| `GET` | `/rooms/{room_id}/snapshot` | 역할별 | 현재 전체 상태 복구 |
| `PUT` | `/rooms/{room_id}/participants/me/statements` | 참가자 | 문장 3개 생성/전체 교체 |
| `PUT` | `/rooms/{room_id}/rounds/{round_id}/vote` | 참가자 | 투표 등록/변경 |
| `GET` | `/rooms/{room_id}/events` | 역할별 | SSE 상태 변경 알림 |
| `POST` | `/rooms/{room_id}/host/start` | 진행자 | 게임 시작 |
| `POST` | `/rooms/{room_id}/host/rounds/{round_id}/voting/start` | 진행자 | 투표 시작 |
| `POST` | `/rooms/{room_id}/host/rounds/{round_id}/voting/extend` | 진행자 | 투표 시간 연장 |
| `POST` | `/rooms/{room_id}/host/rounds/{round_id}/voting/close` | 진행자 | 투표 조기 마감 |
| `POST` | `/rooms/{room_id}/host/rounds/{round_id}/reveal` | 진행자 | 정답 및 점수 공개 |
| `POST` | `/rooms/{room_id}/host/rounds/{round_id}/skip` | 진행자 | 현재 라운드 건너뛰기 |
| `POST` | `/rooms/{room_id}/host/rounds/next` | 진행자 | 다음 라운드 또는 최종 결과 이동 |
| `POST` | `/rooms/{room_id}/host/pause` | 진행자 | 게임 일시 정지 |
| `POST` | `/rooms/{room_id}/host/resume` | 진행자 | 게임 재개 |
| `POST` | `/rooms/{room_id}/host/cancel` | 진행자 | 시작 전 게임 취소 |
| `POST` | `/rooms/{room_id}/host/finish` | 진행자 | 진행 중 게임 조기 종료 |
| `POST` | `/rooms/{room_id}/host/participants/{participant_id}/kick` | 진행자 | 시작 전 참가자 내보내기 |
...
- `name`: trim 후 1~40자
- `max_participants`: 정수 2~100
- `statement_max_length`: 정수 20~200. 최소 길이는 MVP에서 5자로 고정
- `voting_duration_seconds`: 정수 15~180
...
`join_url`에는 방 코드만 포함하고 서명 토큰이나 진행자 권한 정보를 넣지 않는다.
...
- `LOBBY`, `SUBMISSION`, `READY`에서만 참가할 수 있다. 게임 시작 후에는 관전자 입장으로 전환하지 않고 거절한다.
...
`GET /api/v1/rooms/{room_id}/snapshot?audience={audience}`

`audience`:

- `participant`: 해당 방 참가자 쿠키 필수
- `host`: 해당 방 진행자 쿠키 필수
...
- `PAUSED`에서는 `room.paused_from_status`와 현재 라운드를 포함하며 `voting_ends_at`은 재개 후 갱신한다.

### 5.5 문장 저장/수정

`PUT /api/v1/rooms/{room_id}/participants/me/statements`
...
`PUT /api/v1/rooms/{room_id}/rounds/{round_id}/vote`
...
- 현재 방/라운드가 `VOTING`이고 서버 시간이 `voting_ends_at` 전일 때만 성공한다.
...
모든 endpoint는 해당 방의 진행자 쿠키와 `Idempotency-Key`를 요구한다. 성공 응답은 모두 `204 No Content`이며 이후 snapshot 또는 SSE로 새 상태를 확인한다.

### 6.1 게임 시작

`POST /api/v1/rooms/{room_id}/host/start`
...
`POST /api/v1/rooms/{room_id}/host/rounds/{round_id}/voting/start`
...
`POST /api/v1/rooms/{room_id}/host/rounds/{round_id}/voting/extend`
...
`POST /api/v1/rooms/{room_id}/host/rounds/{round_id}/voting/close`
...
`POST /api/v1/rooms/{room_id}/host/rounds/{round_id}/reveal`
...
`POST /api/v1/rooms/{room_id}/host/rounds/{round_id}/skip`

- 허용 상태: `ROUND_INTRO`
- 해당 라운드는 점수 변화 없이 skipped로 기록하고 다음 라운드를 `ROUND_INTRO`로 연다.
- 마지막 라운드였다면 `FINISHED`로 전환한다.

### 6.7 다음 라운드

`POST /api/v1/rooms/{room_id}/host/rounds/next`

- 허용 상태: `RESULT`
- 다음 발표자가 있으면 다음 라운드를 `ROUND_INTRO`로 연다.
- 마지막 라운드였다면 `FINISHED`로 전환하고 최종 leaderboard를 확정한다.

### 6.8 일시 정지/재개

`POST /api/v1/rooms/{room_id}/host/pause`

- 허용 상태: `ROUND_INTRO`, `VOTING`, `VOTE_CLOSED`, `RESULT`
- 이전 상태를 `paused_from_status`로 기록한다.
- `VOTING` 중이면 서버 기준 남은 시간을 저장하고 자동 마감을 무효화한다.

`POST /api/v1/rooms/{room_id}/host/resume`
...
`POST /api/v1/rooms/{room_id}/host/cancel`

- 허용 상태: `LOBBY`, `SUBMISSION`, `READY`
- 상태를 `CANCELLED`로 바꾸고 연결된 클라이언트에 알린 뒤 정리 정책을 시작한다.

`POST /api/v1/rooms/{room_id}/host/finish`

- 허용 상태: `ROUND_INTRO`, `VOTING`, `VOTE_CLOSED`, `RESULT`, `PAUSED`
- 아직 공개하지 않은 현재 라운드에는 점수를 반영하지 않는다.
- 완료된 라운드의 누적 점수로 leaderboard를 계산하고 `FINISHED`로 전환한다.

### 6.10 참가자 내보내기

`POST /api/v1/rooms/{room_id}/host/participants/{participant_id}/kick`

- 허용 상태: `LOBBY`, `SUBMISSION`, `READY`
- 참가자, 제출 문장, 세션을 제거하고 인원/준비 상태를 재평가한다.
- 게임 시작 후에는 발표 순서와 집계를 보호하기 위해 `INVALID_STATE_TRANSITION`으로 거절한다.

## 7. SSE 실시간 이벤트

### 7.1 연결

`GET /api/v1/rooms/{room_id}/events?audience={audience}`
...
- 이벤트 `id`는 방 version과 같거나 재개 가능한 별도 단조 증가 ID다.
- 브라우저가 보내는 `Last-Event-ID` 이후 이벤트를 재전송할 수 있으면 재전송한다.
- 보관 범위를 벗어난 이벤트라면 `room.sync_required`를 보내 snapshot 재조회를 유도한다.

```text
id: 18
event: vote.progress_changed
retry: 3000
data: {"event_id":"evt_01J7K...","room_id":"room_01J7J...","version":18,"occurred_at":"2026-09-05T10:30:01.000Z"}
...
| `room.sync_required` | 서버가 이벤트 연속성을 보장할 수 없을 때 |
...
- `FINISHED`, `CANCELLED`, `EXPIRED` 상태는 연결된 클라이언트가 마지막 화면/안내를 받을 짧은 grace period 동안만 조회 가능하게 유지할 수 있다.
- grace period 종료 후 방, 참가자, 문장, 라운드, 투표, 점수, 세션 및 idempotency record를 함께 삭제한다.
