...
export function DisplayPage() {
  ...
  const { snapshot, error, loading, connection, refresh } = useTtfGameSnapshot(gameId, 'display')
  ...
  return (
    <div className="display-screen">
      ...
      <main>
        ...
        {['LOBBY', 'SUBMISSION', 'READY'].includes(snapshot.game.status) ? (
          <section className="display-lobby">
            <div className="display-lobby__copy">
              ...
              <div className="display-ready-count"><strong>{snapshot.game.ready_count}</strong><span>/ {snapshot.room.participant_count}명 준비 완료</span></div>
            </div>
        ...
        {round && snapshot.game.status === 'RESULT' && round.result ? (
          <section className="display-result"><RoundHeading round={round} /><ResultPanel result={round.result} /></section>
        ) : null}

        {snapshot.game.status === 'FINISHED' ? <div className="display-leaderboard"><Leaderboard entries={snapshot.leaderboard ?? []} /></div> : null}

        {snapshot.game.status === 'CANCELLED' || snapshot.room.status === 'EXPIRED' ? (
